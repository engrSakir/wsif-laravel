                ...
                <div class="justify-content-md-end">
                    <div class="row justify-content-between align-items-center">
                        <div class="col-sm-12 col-md-auto mt-4 mt-sm-0">
                            <div class="d-grid">
                                <a href="{{ route('news.create') }}" class="btn btn-primary">+ Add New One</a>
                            </div>
                        </div><!--end col-->
                    </div><!--end row-->
                </div>
                            ...
                            <td class="py-3">
                                <a href="#" class="text-primary">
                                    <div class="d-flex align-items-center">
                                        <img src="{{ asset($news->image) }}" class="avatar avatar-md-sm rounded-circle shadow" alt="">
                                        <span class="ms-2">{{ $news->name }}</span> &nbsp; <img src="{{ $team->flag->url ?? '' }}" alt="">
                                    </div>
                                </a>
                            </td>
                            <td>{{ $news->title }}</td>
                            <td>{{ $news->created_at->format('h:i A d-M-Y') }}</td>
                            <td class="text-end">
                                <a href="{{ route('news.edit', $news) }}" class="btn btn-soft-primary"> Edit </a>
                                <button class="btn btn-soft-danger delete-btn" value="{{ route('news.destroy', $news) }}"> Delete </button>
                            </td>
                        </tr>
                        @endforeach
                        </tbody>
